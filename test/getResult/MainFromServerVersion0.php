...
class Main
{
    ...
    public function connectToDB(string $host, int $port, string $db, string $user, string $password, bool $presistent = false)
    {
        ...
        while (true) {
            ...
            try {
                ...
                sleep(1);
                echo "[ERROR] Can not connect to db: $e" . PHP_EOL;
                continue;
            }
    ...
    /*
        ...
        (
            ...
            [timestamp] => 1643023591837
            [datetime] => 2022-01-24T11:26:31.837Z
            [nonce] =>
            [base] => BTC
            [quote] => USDN
            [amount_precision] => 8
    ...
    public function getOrderbook(string $symbol, int $depth = MAX_DEPTH)
    {
        try {
            ...
            if (!empty($this->ip)) {
                ...
                $ip = (next($this->ip) == null) ? reset($this->ip) : current($this->ip);

                $this->exchange->curlopt_interface = $ip;
...
//        echo '<pre>';
    ...
    public function getKeys($exchanges, $assets)
    {
